...
export default function Home() {
  ...
  return (
    // pl-30 = largura do aside (w-30), para o conteúdo não ficar por baixo dele
    <main className="fixed inset-x-0 top-0 bottom-18 z-10 flex flex-col overflow-hidden bg-white pl-30 lg:relative lg:inset-auto lg:z-auto lg:mx-auto lg:block lg:min-h-screen lg:w-5xl lg:overflow-visible lg:px-0 lg:pb-24 lg:pt-0 lg:mt-35 xl:mt-40 2xl:w-7xl 2xl:mt-45">
      {/*
        Sidebar mobile: vai de top-0 até bottom-18 (mesma altura da navbar
        inferior). A borda direita fica no próprio aside, por isso acompanha
        a altura toda e não apenas a altura da lista de categorias.
      */}
      <aside className="fixed left-0 top-0 bottom-18 z-30 flex w-30 flex-col border-r border-slate-200/80 bg-white/70 backdrop-blur-xl lg:hidden">
          ...
          {[null, ...productGroups].map((groupType) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-20 w-full items-center gap-2 border-l-2 px-3 text-left text-xs font-medium transition-colors ${isActive ? "border-blue-500 bg-white/85 text-slate-900" : "border-transparent text-slate-500 hover:bg-white/45"}`}
              >
                ...
                {products.map((product) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/*
                        Mobile: o padding vem só do p-2 do article. Sem pt/pb
                        extra e sem space-y no conteúdo, e centrado na vertical,
                        para o espaço em cima e em baixo ficar igual.
                      */}
