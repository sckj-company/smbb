...
import { ArrowLeft, Check, Menu, Plus, Search, ShoppingCart, X } from "lucide-react"
  ...
  return (
    <main className="test-store min-h-screen bg-[#f7f7f4] text-[#172033]">
      <div className="mx-auto flex min-h-screen max-w-[1440px] flex-col px-3 pb-24 pt-4 sm:px-6 sm:pt-7">
        ...
        <div className="flex min-h-0 flex-1 items-start gap-3 sm:gap-5">
          ...
          >
            ...
            >
              <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl text-sm font-black ${!selectedGroup ? "bg-white/20" : "bg-blue-50 text-blue-500"}`}>
                ∑
              </span>
              <span className="text-xs font-bold leading-tight sm:text-sm">Todos os produtos</span>
            </button>
            ...
            {productGroups.map((groupType) => {
              ...
              return (
                ...
                >
                  <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl text-sm font-black ${active ? "bg-white/20" : "bg-blue-50 text-blue-500"}`}>
          ...
          <section className="min-w-0 flex-1">
            <div className="mb-4 flex items-center gap-2 sm:mb-5">
              ...
              <h1 className="truncate text-xl font-black tracking-tight sm:text-2xl">{selectedLabel}</h1>
            </div>

            {isLoading && <Loader />}
            {error && <p className="rounded-2xl bg-red-50 p-4 text-sm text-red-600">Não foi possível carregar os produtos.</p>}
            {!isLoading && !error && visibleProducts.length === 0 && (
              <p className="rounded-2xl bg-white p-6 text-sm text-slate-500 shadow-sm">Nenhum produto encontrado.</p>
            )}
            ...
            <div className="space-y-3 sm:space-y-4">
              {visibleProducts.map((product) => {
                ...
                return (
                  <article key={product.id} className="flex min-h-[9rem] overflow-hidden rounded-[1.35rem] bg-white shadow-[0_2px_6px_rgba(24,33,49,0.09)] transition hover:shadow-[0_8px_20px_rgba(24,33,49,0.11)] sm:min-h-[10.5rem]">
                    <Link href={`/${product.id}`} className="relative w-[38%] min-w-[7.5rem] bg-[#fff4da] sm:w-[34%] sm:min-w-[10rem]">
                      <Image src={product.image} alt={productName} fill sizes="(max-width: 640px) 38vw, 260px" className="object-cover" />
                    </Link>
                    <div className="flex min-w-0 flex-1 flex-col justify-between p-3 sm:p-5">
                      <div>
                        <h2 className="line-clamp-2 text-sm font-extrabold leading-tight text-slate-800 sm:text-lg">{productName}</h2>
                        <p className="mt-1 line-clamp-1 text-[11px] text-slate-400 sm:text-xs">{product.brand || "SMBB"}</p>
                      </div>
                      <div className="mt-2 flex items-end justify-between gap-2 border-t border-dashed border-slate-200 pt-2 sm:pt-3">
                        <p className="text-base font-black text-[#0b5aed] sm:text-xl">{formatKz(product.price)}</p>
                        <button
                          type="button"
      ...
      >
        ...
        <span>{totalItems > 0 ? `${totalItems} ${totalItems === 1 ? "item" : "itens"}` : "Carrinho"}</span>
