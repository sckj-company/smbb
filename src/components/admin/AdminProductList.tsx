...
const fetcher = (url: string) =>
  fetch(url).then((r) => r.json() as Promise<Product[]>);
export default function AdminProductList() {
  ...
  const visible = products.filter(
    (p) =>
      (!selectedGroup || p.groupType === selectedGroup) &&
      `${p.name} ${p.brand} ${p.groupType}`
        .toLowerCase()
  ...
  async function remove(id: string) {
    ...
    if (!response.ok) {
      ...
      setError(result.error ?? "Não foi possível apagar o produto.");
      return;
    }
    await mutate();
  }
